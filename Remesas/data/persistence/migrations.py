...
def _postgres_ddl(sql: str) -> list[str]:
    sql = re.sub(
...
def migrate(conn) -> None:
    """Aplica en orden migraciones atómicas del esquema ``liquidaciones``."""
    conn.execute("CREATE SCHEMA IF NOT EXISTS liquidaciones")
    conn.execute("CREATE TABLE IF NOT EXISTS liquidaciones.schema_version(version INTEGER PRIMARY KEY,name TEXT NOT NULL,applied_at TIMESTAMPTZ NOT NULL)")
    done={r[0] for r in conn.execute("SELECT version FROM liquidaciones.schema_version")}
    for version,name,sql in MIGRATIONS:
        if version in done: continue
        try:
            if version == 9:
                pass  # cod_art nace como TEXT en PostgreSQL.
            elif version == 13:
                ...
                for statement in _postgres_ddl(sql):
                    conn.execute(statement)
            now=utcnow()
            conn.executemany("INSERT INTO liquidation_prefixes(crop,prefix,created_at,updated_at) VALUES(?,?,?,?) ON CONFLICT (crop) DO NOTHING", ((c,p,now,now) for c,p in PREFIXES))
            conn.execute("INSERT INTO liquidaciones.schema_version VALUES(?,?,?)",(version,name,now))
        except Exception:
            raise
