...
def test_exporter_creates_required_sheets_continuous_detail_and_incidents(tmp_path):
    ...
    params = wb["Parámetros"]
    assert [params.cell(row, 2).data_type for row in (7, 8, 9, 12)] == ["n", "n", "n", "n"]
    assert all("yy" not in params.cell(row, 2).number_format.lower() for row in (7, 8, 9, 12))
...
def test_independent_consolidated_exporter_handles_multiple_remittances(tmp_path):
    ...
    export_consolidated_liquidation_summary(
        [batch_result(remittance(2204)), batch_result(remittance(2205))], [], output,
        campaign="2026", company="1", crop="CITRICOS",
        execution_started_at=datetime(2026, 7, 16, 7, 45),
        execution_finished_at=datetime(2026, 7, 16, 7, 46),
    )
    assert load_workbook(output).sheetnames == [
        "Resumen general", "Detalle por socio", "Detalle calibres",
        "Advertencias", "Remesas fallidas",
    ]
