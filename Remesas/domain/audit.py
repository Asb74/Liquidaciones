...
class AuditLogger:
    ...
    def audit_fiscal_regime(self, member: MemberLiquidation) -> None:
        ...
        self.line(f"vat_factor={member.vat_factor}")
        self.line(f"Base={member.taxable_base}")
        self.line(f"raw_amount_after_vat={member.raw_amount_after_vat}")
        self.line(f"IVAImporte={member.vat_amount}")
        self.line(f"amount_after_vat={member.amount_after_vat}")
        self.line(f"Ret={member.withholding_rate}")
        self.line(f"withholding_factor={member.withholding_factor}")
        self.line("Retención aplicada sobre amount_after_vat, no sobre taxable_base")
        self.line(f"raw_total_amount={member.raw_total_amount}")
        self.line(f"RetImporte={member.withholding_amount}")
        self.line(f"Total={member.total_amount}")
        self.line(f"effective_net_kg={member.effective_net_kg}")
        self.line(f"final_average_price={member.final_average_price}")
        self.line(f"preview_total={member.total_amount} model_total={member.total_amount} excel_total={member.total_amount} aligned=True")
