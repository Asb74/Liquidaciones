...
class LiquidacionCalculator:
    ...
    def calculate(self, deliveries: list[Delivery], remesa: Remesa | None) -> LiquidationCalculationResult:
        ...
        for (socio, name, variety), data in sorted(grouped.items()):
            ...
            member=MemberLiquidation(member_id=socio, member_name=name, variety=variety, delivery_count=data["count"], net_deliveries=data["net"], net_commercial=commercial_kg, net_waste=data["des"]+data["mesa"], net_rotten=data["pod"], grades=tuple(grades), commercial_amount=round_money(commercial_amount), destruction_amount=des_amount, table_destruction_amount=mesa_amount, rotten_amount=pod_amount, destruction_price=round_price(prices["PDESTRIO"]), table_destruction_price=round_price(prices["PDMESA"]), rotten_price=round_price(prices["PPODRIDO"]), national_market_price=round_price(prices["PDESTRIO"]), rotten_leaves_price=round_price(prices["PPODRIDO"]), gross_amount=gross, detected_collection_amount=detected_collection, collection_amount=round_money(collection), detected_transport_amount=detected_transport, transport_amount=round_money(transport), quality_amount=qamount, globalgap_amount=Decimal("0"), hectare_fee_amount=Decimal("0"), effective_net_kg=data["net"], quality_rate=qrate, quality_source=qsource, taxable_base=None, commercial_average_price=avg, final_average_price=None, warnings=tuple(line_warnings), statuses=statuses, source_deliveries=tuple(data["deliveries"]))
