...
def export_liquidation_summary(result: LiquidationResult, path: Path) -> Path:
    ...
    entregas_ws.append(entregas_headers)
    seen_delivery_rows = set()
    for member in result.member_results:
        for row in getattr(member, "hectare_fee_delivery_audit", ()):
            key = tuple(row.get(h) for h in entregas_headers)
            if key in seen_delivery_rows:
                continue
            seen_delivery_rows.add(key)
            entregas_ws.append([row.get(h) if h not in {"Neto", "NetoPartida", "NetoEfectivo"} else _number(row.get(h), h) for h in entregas_headers])
