...
def export_member_pdf(result: LiquidationResult, path: Path) -> Path:
    ...
    for idx, m in enumerate(result.member_results):
        ...
        story.append(_table(Table, TableStyle, colors, [["Hectáreas", "Precio €/ha", "Cuota total", "Kilos efectivos totales", "Proporción €/kg", "Cuota parcial"], [format_decimal_es(m.applicable_hectares,4), format_currency_es(m.hectare_fee_price), format_currency_es(m.hectare_fee_total_member), format_integer_es(m.hectare_fee_total_effective_kg), (format_price_es(m.hectare_fee_rate_per_kg) if m.hectare_fee_rate_per_kg is not None else ""), fee_text]])); story.append(Spacer(1, 10))
